...
void processTexturePattern(const Config &config, const TexturePattern &pattern)
{
    ...
    for (const auto &file : files)
    {
        ...
        std::string filename = (last_sep != std::string::npos) ? file.substr(last_sep + 1) : file;
...
#pragma omp critical
        {
            printf("\n[%zu/%zu] 线程 %d 处理: %s (UV: %u,%u)\n",
                   tile_idx + 1, tiles.size(), omp_get_thread_num(),
                   tile.filepath.c_str(), tile.u, tile.v);
        }
